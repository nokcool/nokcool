...
public class issue {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int count = getCount(word2, word1);//计算重复字数
        int result=count*100000/word2.length();
        int[] arr = getInts(result);//计算重复率，将结果存入数组
        writeResult(arr, out);//将结果写入文件中
        System.out.println(word2.length());
    }

    public static void writeResult(int[] arr, String out) throws IOException {
    ...
    public static int[] getInts(int result) {
        int []arr=new int[4];
        for(int i=0;i<3;i++){
            arr[i]= result %10;
            result /=10;
